...
export class TelParametersConfiguration extends React.Component<ConnectedState & ConnectedDispatch, State> {
    state: State;
    props: Props;
    ...
    render() {
        ...
        return (
            ...
            >
                ...
                <Text style={[styles.note, { paddingTop: scale(20) }]}>Configurer gratuitement votre service.</Text>
                <TouchableHighlight underlayColor="rgba(212,0,122,0.1)" onPress={() => Communications.phonecall('0478505050', true)}>
                    <Text style={styles.note}>
                        Si vous souhaitez désactiver un paramètre, <Text style={{ textDecorationLine: 'underline' }}>appelez notre service client.</Text>
                    </Text>
                </TouchableHighlight>
                <OptionsSelector hideOptionDescription={true} options={phoneTelOptions} onActivateOptions={activateOption} key={"option-selector"} />
            </MyScrollView>
        );
    }
}
...
const mapDispatchToProps = (dispatch: Dispatch<any>): ConnectedDispatch => ({
    ...
    activateOption: (option: Models.OptionViewResource) => {
        ...
        return dispatch(customerOptionsActions.activateCustomerOptions(
            [option], OptionActivationOrigins.TelParametersConfiguration));
    },
});
...
export default connect(mapStateToProps, mapDispatchToProps)(TelParametersConfiguration);
